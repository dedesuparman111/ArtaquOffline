...
export const ProModal: React.FC<ProModalProps> = ({
  ...
  // Admin / Seller Generator State
  const [showAdminTool, setShowAdminTool] = useState(false);
  const [targetDevIdInput, setTargetDevIdInput] = useState(licenseInfo.deviceId);
  const [custCodeInput, setCustCodeInput] = useState('PRO1');
  ...
  const handleGenerateKey = () => {
    const key = licenseService.generateKey(genKeyType, targetDevIdInput, custCodeInput);
    setGeneratedKey(key);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/70 backdrop-blur-md animate-fade-in overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-xl max-h-[92vh] flex flex-col overflow-hidden border border-slate-100 dark:border-slate-800 transition-all my-auto">
        ...
        <div className="p-5 sm:p-6 overflow-y-auto space-y-6 flex-1 text-slate-800 dark:text-slate-200">
          ...
          {/* Built-in Key Generator (Tool Khusus Penjual / Developer) */}
          <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
            <button
              type="button"
              onClick={() => setShowAdminTool(!showAdminTool)}
              className="text-[11px] font-bold text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 flex items-center gap-1.5 transition cursor-pointer"
            >
              <Wrench className="w-3.5 h-3.5" />
              <span>{showAdminTool ? 'Sembunyikan Generator Lisensi (Admin)' : 'Alat Pembuat Lisensi (Untuk Penjual/Admin)'}</span>
            </button>

            {showAdminTool && (
              <div className="mt-3 p-4 rounded-2xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-3 animate-fade-in text-xs">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                    <ShieldCheck className="w-4 h-4 text-primary" />
                    <span>Generator Serial Key Offline</span>
                  </span>
                  <span className="text-[9px] font-mono px-2 py-0.5 bg-primary/10 text-primary rounded-md">
                    Admin Tools
                  </span>
                </div>

                <p className="text-[11px] text-slate-500 leading-relaxed">
                  Gunakan alat ini untuk membuat kode lisensi bagi pembeli tanpa membutuhkan koneksi server.
                </p>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Tipe Lisensi</label>
                    <select
                      value={genKeyType}
                    ...
                    >
                      ...
                      <option value="universal">Universal (Bisa untuk Semua Perangkat)</option>
                    </select>
                  </div>

                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Kode / Nama Pembeli</label>
                    <input
                      type="text"
                      value={custCodeInput}
                      onChange={(e) => setCustCodeInput(e.target.value)}
                      placeholder="Contoh: DEDE / VIP1"
                      className="w-full p-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs font-semibold uppercase"
                    />
                  </div>
                </div>
                ...
                {genKeyType === 'device' && (
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Target Device ID Pembeli</label>
                    <input
                      type="text"
                      value={targetDevIdInput}
                      onChange={(e) => setTargetDevIdInput(e.target.value)}
                      placeholder="Contoh: ARTA-ABCD-1234"
                      className="w-full p-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 font-mono text-xs font-bold"
                    />
                  </div>
                )}
                ...
                <button
                  ...
                  className="w-full py-2 bg-primary hover:bg-primary/90 text-white rounded-xl font-bold transition flex items-center justify-center gap-1.5 cursor-pointer"
                >
                  <Zap className="w-3.5 h-3.5" />
                  <span>Generate Serial Key Pembeli</span>
                </button>

                {generatedKey && (
                  <div className="p-3 rounded-xl bg-white dark:bg-slate-900 border border-primary/40 space-y-1.5">
                    <span className="text-[10px] font-bold text-slate-400 block uppercase">Serial Key Siap Dikirim:</span>
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-mono font-black text-primary text-xs sm:text-sm tracking-wider select-all truncate">
                        {generatedKey}
                      </span>
                      <button
                        type="button"
                        onClick={handleCopyGeneratedKey}
                        className="px-2.5 py-1 bg-primary text-white rounded-lg text-xs font-bold shrink-0 cursor-pointer"
                      >
                        {copiedGenKey ? 'Disalin' : 'Salin'}
                      </button>
                    </div>
                  </div>
                )}
        ...
        {/* Footer */}
        <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-950/60 flex items-center justify-end">
          <button
            type="button"
            onClick={onClose}
