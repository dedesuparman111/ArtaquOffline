...
import { Target, Plus, Wallet, Calendar as CalendarIcon, CheckCircle2, MoreVertical, Edit, Trash2, Crown } from 'lucide-react';
...
export const Savings: React.FC<SavingsProps> = ({
  ...
  const handleOpenAddModal = () => {
    if (!isPro && savingsGoals.length >= FREE_LIMITS.SAVINGS) {
      if (onOpenProModal) {
        onOpenProModal(`Batas versi Gratis tercapai (${FREE_LIMITS.SAVINGS} target). Upgrade ke ArtaQu PRO untuk membuat target tabungan tanpa batas!`);
      }
      return;
    }
  ...
  return (
    <div className="space-y-6 pb-24 animate-fade-in">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            {isPro ? (
              ...
              <button
                type="button"
                onClick={() => onOpenProModal && onOpenProModal('Upgrade ke PRO untuk membuat target tabungan tanpa batas.')}
              ...
              >
                {savingsGoals.length}/{FREE_LIMITS.SAVINGS} Free
              </button>
            )}
          </div>
      ...
      {/* MODAL */}
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-4 pt-10 sm:pt-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in">
